...
const navTiles = [
  {
    ...
    detail:
      "Paid vs unpaid · Sent vs unsent · Overdue vs current · On-time vs late",
  },
  {
    to: "/customers",
    icon: Users,
      ...
      <section className="space-y-5">
        ...
        {overview.isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-6">
            <KpiCard
              label="Customers"
              value={formatInteger(overview.data.customer_count)}
            />
            <KpiCard
              label="Invoices"
              value={formatInteger(overview.data.invoice_count)}
            />
            <KpiCard
              label="Payments"
              value={formatInteger(overview.data.payment_count)}
            />
            <KpiCard
              label="Outstanding"
            ...
            <KpiCard
              label="Invoiced"
              value={formatCurrency(overview.data.total_invoiced)}
            />
            <KpiCard
